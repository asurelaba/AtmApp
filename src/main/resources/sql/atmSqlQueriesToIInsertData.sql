DELETE
FROM `atm_app`.`transactions`;
DELETE
FROM `atm_app`.`events`;
DELETE
FROM `atm_app`.`event_types`;
DELETE
FROM `atm_app`.`accounts`;
DELETE
FROM `atm_app`.`cards`;
DELETE
FROM `atm_app`.`card_types`;
DELETE
FROM `atm_app`.`users`;
DELETE
FROM `atm_app`.`user_roles`;
DELETE
FROM `atm_app`.`persons`;
ALTER TABLE `atm_app`.`transactions`
    AUTO_INCREMENT = 1;
ALTER TABLE `atm_app`.`events`
    AUTO_INCREMENT = 1;
ALTER TABLE `atm_app`.`event_types`
    AUTO_INCREMENT = 1;
ALTER TABLE `atm_app`.`accounts`
    AUTO_INCREMENT = 1;
ALTER TABLE `atm_app`.`cards`
    AUTO_INCREMENT = 1;
ALTER TABLE `atm_app`.`card_types`
    AUTO_INCREMENT = 1;
ALTER TABLE `atm_app`.`users`
    AUTO_INCREMENT = 1;
ALTER TABLE `atm_app`.`user_roles`
    AUTO_INCREMENT = 1;
ALTER TABLE `atm_app`.`persons`
    AUTO_INCREMENT = 1;

-- Insert card types
INSERT INTO `atm_app`.`card_types` (`name`)
VALUES ('AdministratorCard'),
       ('ClientCard');

-- Insert user roles
INSERT INTO `atm_app`.`user_roles` (`name`)
...
SELECT 1111111111111110 + `person_id`,
...
INSERT INTO `atm_app`.`cards` (`card_number`, `pin`, `status`, `type_id`, `user_id`)
SELECT 2222222222222210 + `person_id`,
       FLOOR(RAND() * (9999 - 1000 + 1)) + 1000,
       'active',
       2,
       `user_id`
