...
import com.solvd.views.atm.admin.AdminTransactionView;
...
public class AdminTransactionController implements IFeatureController {
    ...
    private void handleTransactionsByStatus() {
        ...
        // Display query
        if (input != null) {
            List<Transaction> transactions = new TransactionService().getTransactionsByStatus(
                input);
    ...
    private void handleTransactionsByCardNumber() {
        ...
        // Display query
        if (input != 0) {
            List<Transaction> transactions = new TransactionService().getTransactionsByCardNumber(
                input);
            if (transactions != null && !transactions.isEmpty()) {
                view.displayBody("Transactions by " + queryType + ":");
                formatObject(transactions);
    ...
    private void handleTransactionsByDateRange() {
        ...
        if (from != null && to != null) {
            List<Transaction> transactions = new TransactionService().getTransactionsByDateRange(
                from, to);
            if (transactions != null && !transactions.isEmpty()) {
                view.displayBody("Transactions by " + queryType + ":");
                formatObject(transactions);
    ...
    private void handleTransactionsByUserId() {
        ...
        // Display query
        if (input != 0) {
            List<Transaction> transactions = new TransactionService().getTransactionsByUserId(
                input);
            if (transactions != null && !transactions.isEmpty()) {
                view.displayBody("Transactions by " + queryType + ":");
                formatObject(transactions);
    ...
    private void formatObject(List<Transaction> transactions) {
        ...
        // Header
        view.displayBody(
            view.centerAndTrim("Date", width2) + " | " + view.centerAndTrim("Transaction", width2) +
                " | " + view.centerAndTrim("Status", width) + " | " + view.centerAndTrim("Amount",
                width) +
                " | " + view.centerAndTrim("Balance", width) + " | " + view.centerAndTrim(
                "Transaction id", width)
                + " | " + view.centerAndTrim("Event Id", width) + " | " + view.centerAndTrim(
                "Account Id", width)
                + " | " + view.centerAndTrim("Card Number", width) + " | " + view.centerAndTrim(
                "User Id", width)
                + " | " + view.centerAndTrim("First Name", width) + " | " + view.centerAndTrim(
                "Last Name", width)
        );
        ...
        for (Transaction t : transactions) {
            ...
            balance = String.valueOf(
                new AccountService().getAccountByUserId(t.getEvent().getCard().getUser().
            ...
            view.displayBody(
                view.centerAndTrim(date, width2) + " | " + view.centerAndTrim(transaction, width2) +
                    ...
                    " | " + view.centerAndTrim(balance, width) + " | " + view.centerAndTrim(
                    transactionId, width) +
                    " | " + view.centerAndTrim(eventId, width) + " | " + view.centerAndTrim(
                    accountId, width) +
                    " | " + view.centerAndTrim(cardNumber, width) + " | " + view.centerAndTrim(
                    userId, width) +
                    " | " + view.centerAndTrim(firstName, width) + " | " + view.centerAndTrim(
                    lastName, width));
        }
    }
    ...
    private void formatObject(Transaction t) {
        ...
        // Data
        view.displayBody(
            view.centerAndTrim(date, width2) + " | " + view.centerAndTrim(transactionName, width2) +
                " | " + view.centerAndTrim(status, width) + " | " + view.centerAndTrim(amount,
                width) +
                " | " + view.centerAndTrim(balance, width) + " | " + view.centerAndTrim(
                transactionId, width) +
                " | " + view.centerAndTrim(eventId, width) + " | " + view.centerAndTrim(accountId,
                width) +
                " | " + view.centerAndTrim(cardNumber, width) + " | " + view.centerAndTrim(userId,
                width) +
                " | " + view.centerAndTrim(firstName, width) + " | " + view.centerAndTrim(lastName,
                width));
    }
